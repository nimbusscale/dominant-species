...
export class AnimalFactoryService {
  ...
  private buildAnimals(): void {
    this.factionRegistryService.factionAssignment$
      .subscribe((factionAssignments) => {
        // To minimize any building an animal several times, we add the animal id to processedFactionIds as quickly as possible /
        const factionIds = new Set<string>(factionAssignments.map((factionAssignment) => factionAssignment.id))
        const factionIdsToProcess = setDifference(factionIds, this.processedFactionIds)
        this.processedFactionIds = new Set<string>([...factionIdsToProcess, ...this.processedFactionIds])
  ...
  private buildAnimal(faction: Faction): void {
    ...
    this.areaRegistrySvc.registeredIds$.pipe(
      combineLatestWith(this.pileRegistryService.registeredIds$),
      filter(([areaIds, pileIds]) =>
          areaIds.has(elementAreaId) && pileIds.has(actionPawnPileId) && pileIds.has(speciesPileId)
      ),
      first()
    ).subscribe(() => {
