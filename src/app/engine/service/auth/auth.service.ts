...
export class AuthService {
  ...
  async login(username: string, password: string): Promise<boolean> {
    ...
    if (authResult) {
      this.localStorageService.setStorageKey(LocalStorageKey.PLAYER_AUTH_DATA, JSON.stringify(this.authResultToPlayerAuth(authResult)));
      return true;
    } else {
      return false;
    }
  ...
  private validatePlayerAuthData(playerAuthData: PlayerAuthData): boolean {
    return playerAuthData.accessTokenExpire < Date.now();
  }
