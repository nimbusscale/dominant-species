...
import {AnimalFactoryService} from '../../../app/game/service/animal-factory.service';
...
describe('AnimalFactoryService', () => {
  let animalFactoryService: AnimalFactoryService;
  ...
  beforeEach(() => {
    ...
    const factionRegistrySvcSpyObj = jasmine.createSpyObj<FactionRegistryService>(
      ...
      ['get'],
      {
        factionAssignment$: of([{ id: AnimalEnum.BIRD, ownerId: 'tester1' }, { id: AnimalEnum.INSECT, ownerId: 'tester2' }]),
      },
    );
    factionRegistrySvcSpyObj.get.and.callFake((animalId): Faction => {
      if (animalId === AnimalEnum.BIRD) {
        return birdFaction
      } else if (animalId === AnimalEnum.INSECT){
    ...
    animalFactoryService = TestBed.inject(AnimalFactoryService);
    mockAnimal = jasmine.createSpyObj<Animal>(['id'])
    animalFactorySpy = spyOn<any>(animalFactoryService, 'animalFactory').and.returnValue(mockAnimal);
  });

  it('should build one animal when single faction assignment', (done) => {
    animalFactoryService.animals$.pipe(skip(1)).subscribe((animals) => {
      expect(animals.length).toEqual(1)
      expect(animalFactorySpy).toHaveBeenCalledWith(jasmine.objectContaining({
    ...
    pileRegistrySvcRegisteredIdsSubject.next(new Set([PileIdEnum.ACTION_PAWN_BIRD, PileIdEnum.SPECIES_BIRD]))
  });
  it('should build multiple animal when multiple faction assignment', (done) => {
    ...
    animalFactoryService.animals$.pipe(skip(2)).subscribe((animals) => {
      expect(animals.length).toEqual(2)
      expect(animalFactorySpy).toHaveBeenCalledTimes(2)
      done()
    })
    areaRegistrySvcRegisteredIdsSubject.next(new Set([AreaIdEnum.BIRD_ELEMENT, AreaIdEnum.INSECT_ELEMENT]))
    pileRegistrySvcRegisteredIdsSubject.next(new Set([PileIdEnum.ACTION_PAWN_BIRD, PileIdEnum.SPECIES_BIRD, PileIdEnum.ACTION_PAWN_INSECT, PileIdEnum.SPECIES_INSECT]))
  });
});
