...
describe('AnimalElements', () => {
  ...
  beforeEach(() => {
    ...
    animalElements = new AnimalElements(testArea, getOrThrow(elementConfigByAnimal, AnimalEnum.BIRD))
  })
  describe('addElement', () => {
    it('can add element', () => {
    ...
    it('throws error when adding too many elements', () => {
      ...
      animalElements.addElement(defaultPieceFactory(ElementEnum.SUN) as ElementPiece)
      animalElements.addElement(defaultPieceFactory(ElementEnum.SUN) as ElementPiece)
      expect(() => {animalElements.addElement(defaultPieceFactory(ElementEnum.SUN) as ElementPiece)}).toThrowError()
    })
  })
  describe('removeElement', () => {
    ...
    it('throws error when trying to remove an added element it does not have', () => {
      expect(() => animalElements.removeElement(defaultPieceFactory(ElementEnum.WATER) as ElementPiece)).toThrowError()
    })
    it('throws error when trying to remove an added element it does not have (matching inherent)', () => {
      expect(() => animalElements.removeElement(defaultPieceFactory(ElementEnum.SEED) as ElementPiece)).toThrowError()
    })
  })
